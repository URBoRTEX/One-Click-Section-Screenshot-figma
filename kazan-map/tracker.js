(()=>{
  ...
  let app=null,map=null,panel=null,toolButton=null,savedLayer=null,currentLine=null,showTracks=true,tracking=false,current=null,cloudSnapshot=null,timer=null;
  ...
  function startTracking(){
    ...
    tracking=true;app.startLocation();if(app.getPosition())addPoint({...app.getPosition(),accuracy:30,timestamp:Date.now()});timer=setInterval(updateUi,1000);updateUi();createLiveBadge().textContent='● Запись прогулки';
  }

  async function stopTracking(){
    if(!tracking||!current)return;tracking=false;clearInterval(timer);timer=null;current.ended_at=new Date().toISOString();current.duration_s=Math.max(1,Math.round((new Date(current.ended_at)-new Date(current.started_at))/1000));
    if(current.points.length>1){
      const record={...current,synced:false};localTracks.push(record);saveJson(LOCAL_TRACKS_KEY,localTracks);
      if(window.KAZAN_CLOUD?.isEditor()&&cloudSnapshot?.schemaReady!==false){
        try{const saved=await window.KAZAN_CLOUD.saveTrack(record);record.synced=true;record.cloud_id=saved.id;saveJson(LOCAL_TRACKS_KEY,localTracks);await window.KAZAN_CLOUD.refresh();}catch(error){alert(error?.message||'Трек сохранён на телефоне, но не загружен в облако.');}
      }
    }
  ...
  function addPoint(point){
    ...
    const normalized={lat:Number(point.lat),lng:Number(point.lng),accuracy:Number(point.accuracy)||0,timestamp:Number(point.timestamp)||Date.now()};
  ...
  function renderTracks(){
    if(!map)return;if(savedLayer)map.removeLayer(savedLayer);savedLayer=L.layerGroup();
    if(showTracks){allTracksForDay(dayKey()).forEach(track=>{const points=Array.isArray(track.points)?track.points:[];if(points.length<2)return;L.polyline(points.map(point=>[point.lat,point.lng]),{color:'#7c3aed',weight:5,opacity:.82,lineCap:'round'}).bindTooltip(`${formatDistance(track.distance_m||0)} · ${formatDuration(track.duration_s||0)}`).addTo(savedLayer);});savedLayer.addTo(map);}
    drawCurrent();
  }

  function aggregate(){
    const key=dayKey(),tracks=allTracksForDay(key);let distance=tracks.reduce((sum,track)=>sum+Number(track.distance_m||0),0),duration=tracks.reduce((sum,track)=>sum+Number(track.duration_s||0),0);
    if(tracking&&current?.day_key===key){distance+=Number(current.distance_m||0);duration+=Math.max(0,Math.round((Date.now()-new Date(current.started_at))/1000));}
    const cloudHealth=cloudSnapshot?.health?.get(key),localSteps=localHealth[key]?.steps,actualSteps=cloudHealth?.steps??localSteps??null;
    return{key,distance,duration,steps:actualSteps??Math.round(distance/.75),estimated:actualSteps===null};
  }
  ...
  async function saveHealth(){
    if(!canEdit()){alert('Сохранение доступно только URBoRTEX.');return;}
    const input=panel.querySelector('#trackerHealthInput'),steps=Math.max(0,Math.round(Number(input.value)||0)),key=dayKey();localHealth[key]={steps,source:'manual',updated_at:new Date().toISOString()};saveJson(LOCAL_HEALTH_KEY,localHealth);
    if(window.KAZAN_CLOUD?.isEditor()&&cloudSnapshot?.schemaReady!==false){try{await window.KAZAN_CLOUD.saveHealth(key,steps,'apple_health_manual');await window.KAZAN_CLOUD.refresh();}catch(error){alert(error?.message||'Шаги сохранены на телефоне, но не в облаке.');}}
    updateUi();
  }
  ...
  function init(application){
    if(app)return;app=application;map=app.map;createUi();renderTracks();updateUi();const restored=loadJson(CURRENT_KEY,null);if(restored?.points?.length){current=restored;tracking=true;app.startLocation();timer=setInterval(updateUi,1000);createLiveBadge();drawCurrent();}
  }

  window.addEventListener('kazan:app-ready',event=>init(event.detail));
  window.addEventListener('kazan:position',event=>addPoint(event.detail));
